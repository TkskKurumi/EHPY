...
class single_page(mydict):
    def __init__(self,url):
        ...
        try:
            iurl=re.findall(r'<img id="img" src="(.+?)" style="',html)[0].replace('&amp;','&')
        except IndexError as e:
            raise Exception("error parsing single_page (%s) (%s)"%(url,e))
        try:
            iurl_f=re.findall(r'<a href="(https://exhentai.org/fullimg.php\?gid=\d+?&amp;page=\d+?&amp;key=.+?)">Download',html)[0]
        except IndexError as e:
            raise Exception("error parsing single_page full img(%s) (%s)"%(url,e))
        self['image_url']=iurl
...
class gallery_page(mydict):
    def __init__(self,url):
        ...
        for url in single_page_urls:
            task=thread_pool.submit(single_page,url)
...
class gallery(mydict):
    def __init__(self,gid,token):
        ...
        self['preview_pic']=re.findall(r'background:transparent url\((.+?)\)',html0)[0]
        
        temp_pagenum=re.findall(r'Showing (\d{1,4}) - (\d{1,4}) of (\d{1,4})',html0)
        ...
        for st in range(1,tot+1,step):
            ...
            task=thread_pool.submit(gallery_page,concat_gallery_url(gid,token,page))
            gallery_page_tasks.append(task)
        gallery_pages=[i.result() for i in gallery_page_tasks]
        
        
        self.pages_flat={"map_referer":{},"image_urls":[],"image_urls_original":[]}
        self.pages=[]
    ...
    def download_tasks(self,pth=None):
        ...
        for idx,_ in enumerate(self.pages):
            tasks.append(thread_pool.submit(self.download_single_page,idx,pth))
        return tasks
